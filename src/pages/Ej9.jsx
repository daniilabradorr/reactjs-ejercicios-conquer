...
* -tambine uso useEffect para persistir 'tasks' en localStorage cada vez que cambian.
* -y las accciones: agregar, marcar/desmarcar (con el toggle que hago), eliminar y limpiar completadas.
*/
...
//useState con función inicializadora: sólo se ejecuta en el primer render
...
<input id="task" value={text} onChange={e=>setText(e.target.value)} placeholder="Nueva tarea…" style={{border:'1px solid #e2e8f0', borderRadius:12, padding:'8px 12px', flex:1, minWidth:220}} />
...
{tasks.map(t=> (
<li key={t.id} style={{display:'flex', alignItems:'center', justifyContent:'space-between', gap:12, background:'#f1f5f9', borderRadius:12, padding:'8px 12px'}}>
